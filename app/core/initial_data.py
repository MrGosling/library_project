...
async def create_initial_authors() -> None:
    authors_data = [
        {"full_name": "Лев Толстой", "birth_year": 1828, "bio": "Один из величайших писателей-романистов мира."},
        {"full_name": "Федор Достоевский", "birth_year": 1821, "bio": "Классик русской литературы и один из лучших романистов мирового значения."},
        {"full_name": "Александр Пушкин", "birth_year": 1799, "bio": "Великий русский поэт, драматург и прозаик."},
        {"full_name": "Михаил Булгаков", "birth_year": 1891, "bio": "Русский писатель советского периода, драматург, филолог."},
        {"full_name": "Антон Чехов", "birth_year": 1860, "bio": "Выдающийся русский писатель, драматург, классик мировой литературы."},
        {"full_name": "Джордж Оруэлл", "birth_year": 1903, "bio": "Британский писатель и публицист, автор антиутопий."},
        {"full_name": "Эрнест Хемингуэй", "birth_year": 1899, "bio": "Американский писатель, журналист, лауреат Нобелевской премии."},
        {"full_name": "Габриэль Гарсиа Маркес", "birth_year": 1927, "bio": "Колумбийский писатель-прозаик, журналист, издатель и политический деятель."},
        {"full_name": "Франц Кафка", "birth_year": 1883, "bio": "Один из крупнейших немецкоязычных писателей XX века."},
        {"full_name": "Рэй Брэдбери", "birth_year": 1920, "bio": "Американский писатель, известный по роману «451 градус по Фаренгейту»."},
    ]

    async with AsyncSessionLocal() as session:
        for author_info in authors_data:
            result = await session.execute(select(Author).where(Author.full_name == author_info["full_name"]))
...
async def create_initial_categories() -> None:
    categories_data = [
        {"name": "Художественная литература", "description": "Литературный род, использующий в качестве единственного материала слова языка."},
        {"name": "Научно-популярная", "description": "Произведения о науке, научных достижениях и об учёных."},
        {"name": "Классика", "description": "Произведения, считающиеся образцовыми для той или иной эпохи."},
        {"name": "Современная проза", "description": "Литературные произведения, созданные в наши дни."},
    ]

    async with AsyncSessionLocal() as session:
        for cat_info in categories_data:
            result = await session.execute(select(Category).where(Category.name == cat_info["name"]))
            if not result.scalars().first():
...
async def create_initial_genres() -> None:
    ...
    async with AsyncSessionLocal() as session:
        for genre_info in genres_data:
            result = await session.execute(select(Genre).where(Genre.name == genre_info["name"]))
            if not result.scalars().first():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for book_info in books_data:
            ...
            result = await session.execute(
                ...
                .options(selectinload(Book.genres), selectinload(Book.categories))
            )
            ...
            if book:
                ...
                for c_name in category_names:
                    if c_name in categories and c_name not in current_cat_names:
